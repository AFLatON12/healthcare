...
const DoctorScheduleManagement = () => {
  ...
  const handleDeleteTimeSlot = async (id) => {
    try {
      await axios.delete(`http://localhost:8080/api/doctor-schedule/${id}`);
              ...
              <TableCell>
                <select
                  ...
                  onChange={async (e) => {
                    ...
                    try {
                      const response = await axios.patch(`http://localhost:8080/api/doctor-schedule/${slot._id}`, {
                        isAvailable: newValue,
                      });
                      setTimeSlots(timeSlots.map(ts => ts._id === slot._id ? response.data : ts));
                    } catch (error) {
