...
const fetchDoctorProfile = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/v1/doctors/${doctorId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const updateDoctorProfile = async () => {
    try {
      await axios.put(`http://localhost:5000/api/v1/doctors/${doctorId}`, doctorProfile, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
        ...
        return profileLoading ? (
          ...
          <Box>
            {editMode ? (
              ...
              <>
                <Typography variant="h6">Name: {doctorProfile?.name}</Typography>
                <Typography variant="h6">Specialization: {doctorProfile?.specialization}</Typography>
