...
const PatientDashboard = () => {
  ...
  const handleCreateAppointment = async () => {
    ...
    try {
      // Convert date and time to startTime and endTime Date objects
      const startDateTime = new Date(`${newAppointment.date}T${newAppointment.time}`);
      // Assuming appointment duration is 1 hour
      ...
      {appointments.length === 0 ? (
        ...
        appointments.map(app => (
          <Paper key={app._id} className="appointment-card" elevation={2} sx={{ p: 2, mb: 2 }}>
            <Typography>Doctor: {app.doctorDetails?.name || app.doctorId}</Typography>
            <Typography>Date: {new Date(app.startTime).toLocaleDateString()}</Typography>
            <Typography>Time: {new Date(app.startTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</Typography>
