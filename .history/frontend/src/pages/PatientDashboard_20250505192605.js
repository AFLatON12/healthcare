        ...
        <>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                // Use current date and time for appointment
                const now = new Date();
                const startDateTime = now;
                const endDateTime = new Date(now.getTime() + 30 * 60000); // 30 minutes duration
                ...
                const created = await createAppointment(appointmentData);
                setAppointments(prev => [...prev, created]);
                alert('Appointment created successfully with selected doctor');
              } catch (error) {
                console.error('Error creating appointment:', error);
                alert('Failed to create appointment');
              }
