...
router.get('/:id/payment', validateObjectId('id'), async (req, res) => {
  try {
    const payment = await appointmentController.getPaymentByAppointment(req.params.id);
    if (!payment) {
...
router.get('/patient/:patientId', validateObjectId('patientId'), async (req, res) => {
  try {
    const appointments = await appointmentController.getAppointmentsByPatient(req.params.patientId);
