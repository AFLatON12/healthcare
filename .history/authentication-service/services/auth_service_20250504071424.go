...
func (s *AuthService) CreateSession(userID string) (string, error) {
	...
	err := redisClient.Set(ctx, sessionID, userID, 24*time.Hour).Err()
	if err != nil {
		return "", err
	}
...
func (s *AuthService) GenerateAPIKey(userID string) (string, error) {
	apiKey := generateSessionID() // Reuse session ID generator for API keys
	err := redisClient.Set(ctx, apiKey, userID, 0).Err() // No expiration for API keys
	if err != nil {
		return "", err
	}
	return apiKey, nil
...
func (s *AuthService) ValidateAPIKey(apiKey string) (string, error) {
	userID, err := redisClient.Get(ctx, apiKey).Result()
	if err != nil {
		return "", err
	}
	return userID, nil
