...
func (s *AuthService) CreateAdmin(creatorID primitive.ObjectID, email, password string, tokenString string) error {
	log.Printf("[DEBUG] Starting CreateAdmin with email: %s", email)

	// Validate token
	claims, err := s.ValidateToken(tokenString)
	if err != nil {
		log.Printf("[ERROR] Token validation failed: %s", err.Error())
		return errors.New("unauthorized: invalid token")
	}
	log.Printf("[DEBUG] Token validated successfully. Claims: %+v", claims)

	// Check permissions
	if !s.HasPermission(claims, "admin:create") {
		log.Printf("[ERROR] Insufficient permissions for user: %s", claims.Email)
		return errors.New("unauthorized: insufficient permissions")
	}
	log.Printf("[DEBUG] Permission check passed for user: %s", claims.Email)
...
func (s *AuthService) RegisterDoctor(doctor *models.Doctor, password string) error {
	...
	// validatePassword method is missing, so implement a simple password validation here
	if len(password) < 6 {
