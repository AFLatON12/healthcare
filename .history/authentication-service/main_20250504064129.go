...
func main() {
	...
	// Public routes with /api/v1/auth prefix
	authGroup := router.Group("/api/v1/auth")
	{
		authGroup.POST("/register", ginAuthHandler.RegisterPatient)
		authGroup.POST("/login", ginAuthHandler.Login)
		authGroup.POST("/initialize-super-admin", ginAuthHandler.InitializeSuperAdmin)
		authGroup.GET("/google", ginSocialAuthHandler.GoogleLogin)
		authGroup.GET("/google/callback", ginSocialAuthHandler.GoogleCallback)
	}

	// Protected routes with /api/v1 prefix
